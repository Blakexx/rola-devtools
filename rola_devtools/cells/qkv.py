...
A QKV cell is ``batch`` sequences of ``tokens`` tokens, ``heads`` heads of width ``dv``, in ``dtype``, and whether the
problem is causal. Every QKV cell draws from one device generator at seed 0, queries then keys then values, so a cell
is its shape.
"""
